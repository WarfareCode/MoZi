...
namespace ns33
{
	...
	public sealed class LicenseFile
	{
		...
		{
			if (!File.Exists(Path.Combine(MyTemplate.GetApp().Info.DirectoryPath, "CommandXLicA.dat")))
			{
				Interaction.MsgBox("找不到许可证文件! 请联系华戍防务，程序将退出...", MsgBoxStyle.OkOnly, null);
				Environment.Exit(0);
			}
			try
			{
                ...
                //解密
                StreamReader streamReader = new StreamReader(Path.Combine(MyTemplate.GetApp().Info.DirectoryPath, "CommandXLicA.dat"));
				string cipherText;
				using (streamReader)
				{
								...
								if (Operators.CompareString(name, "LicenseStart", false) != 0)
								{
									if (Operators.CompareString(name, "LicenseEnd", false) != 0)
									{
										...
										if (Operators.CompareString(innerText2, "Professional", false) == 0)
										{
											...
											GameGeneral.bProfessionEdition = true;
										}
										else
										{
											GameGeneral.bProfessionEdition = false;
										}
									}
									else
									{
				...
				{
					Interaction.MsgBox("您的License授权机器码不符，请联系华戍防务。程序将退出。", MsgBoxStyle.OkOnly, null);
					Environment.Exit(0);
				}
				if (LicenseFile.LicenseStart.CompareTo(DateAndTime.Now) > 0)
				{
					Interaction.MsgBox("您的License还未生效，请联系华戍防务。程序将退出。", MsgBoxStyle.OkOnly, null);
					Environment.Exit(0);
				}
				else if (LicenseFile.LicenseEnd.CompareTo(DateAndTime.Now) < 0)
				{
					Interaction.MsgBox("您的License已经过期，请联系华戍防务。程序将退出", MsgBoxStyle.OkOnly, null);
					Environment.Exit(0);
				}
				if (LicenseFile.LicenseVersion == LicenseFile._LicenseVersion.Evaluation)
				{
					GameGeneral.strVersion += "(试用版)";
				}
			}
			catch (Exception ex)
			{
